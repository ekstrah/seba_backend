...
import { calculateNewRating, calculateRatingAfterDeletion } from "../utils/ratingCalculator.js";
...
export const deleteReview = async (req, res) => {
    try {
        ...
        if (review.type === "farmer") {
            const farmer = await Farmer.findById(review.farmer);
            farmer.rating = calculateRatingAfterDeletion(farmer.rating, review.rating);
            farmer.reviews = farmer.reviews.filter(id => id.toString() !== review._id.toString());
            await farmer.save();
        } else {
            const product = await Product.findById(review.product);
            product.rating = calculateRatingAfterDeletion(product.rating, review.rating);
            product.reviews = product.reviews.filter(id => id.toString() !== review._id.toString());
            await product.save();
        }
